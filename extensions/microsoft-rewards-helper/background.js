...
chrome.action.onClicked.addListener(async tab => {
    ...
    // if we already have the page open in front of us, open the options page
    let urlRegex = new RegExp(`^https?://${domain.replaceAll('.', '\\.')}/`);
    if (urlRegex.test(tab.url)) {
        // try to find the options page in the active window and swap to it,
        // otherwise make a new tab
        [tab] = await chrome.tabs.query({url: chrome.runtime.getURL('options.html'), currentWindow: true});
